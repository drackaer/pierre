...
public class SuperGroupNeuralNetworkEvaluator implements IEvaluator, Serializable {
	
	private static final long serialVersionUID = 1L;
	private List<Double> max, min;
	private BackpropagationRegression learner;
	
	public SuperGroupNeuralNetworkEvaluator() {
		ArrayList<ArrayList<Double>> data = DbManager.getSingleton().getSuperGroupLearnerData();
		findMaxAndMin(data);
	...
	public Evaluation evaluateRecipe(Recipe recipe) {
		...
		try {
			attributeVector = manager.convertRecipeToSuperGroupFormat(recipe);
		...
		return new Evaluation(eval,"This recipe has a super group star rating of ");
	}
